...
echo "📦 Building layer in Linux container (Python 3.13)..."

# Build the layer using Docker with Python 3.13 on Linux AMD64
docker run --rm \
  ...
  bash -c "
    ...
    pip install --quiet Pillow==10.4.0 -t /tmp/python/lib/python3.13/site-packages/ && \
    cd /tmp && \
